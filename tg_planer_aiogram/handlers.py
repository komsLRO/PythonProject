from datetime import datetime, timedelta
from aiogram import Dispatcher
from aiogram.filters import Command, StateFilter
from aiogram.fsm.context import FSMContext
from aiogram.fsm.state import State, StatesGroup
from aiogram.types import Message

import database
import utils


class AddTaskStates(StatesGroup):
    waiting_for_title = State()
    waiting_for_datetime = State()


class ClearAllStates(StatesGroup):
    waiting_for_confirmation = State()


def register_handlers(dp: Dispatcher):
    # Команды
    dp.message.register(on_start, Command("start"))
    dp.message.register(on_help, Command("help"))
    dp.message.register(on_add, Command("add"))
    dp.message.register(on_today, Command("today"))
    dp.message.register(on_week, Command("week"))
    dp.message.register(on_list, Command("list"))
    dp.message.register(on_cleanup, Command("cleanup"))
    dp.message.register(on_clear_all, Command("clear_all"))
    dp.message.register(on_reset_ids, Command("reset_ids"))
    dp.message.register(on_done, Command("done"))
    dp.message.register(on_undo, Command("undo"))
    dp.message.register(on_delete, Command("delete"))
    dp.message.register(on_search, Command("search"))

    # Диалог добавления задачи
    dp.message.register(process_title, StateFilter(AddTaskStates.waiting_for_title))
    dp.message.register(process_datetime, StateFilter(AddTaskStates.waiting_for_datetime))

    # Диалог удаления всех задач
    dp.message.register(process_clear_confirmation, StateFilter(ClearAllStates.waiting_for_confirmation))


async def on_start(message: Message):
    await database.register_user(message.from_user.id, message.from_user.username)

    msg = (
        "🎉 <b>Добро пожаловать в Telegram Planner Bot!</b> 🤖\n\n"
        "Я помогу вам организовать дела и не забыть важные задачи.\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "<b>🚀 БЫСТРЫЙ СТАРТ:</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "1️⃣ <b>/add</b> - Добавьте свою первую задачу\n"
        "2️⃣ <b>/today</b> - Посмотрите задачи на сегодня\n"
        "3️⃣ <b>/help</b> - Подробная справка по всем командам\n\n"

        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "<b>📋 ОСНОВНЫЕ КОМАНДЫ:</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "➕ /add — добавить задачу\n"
        "📅 /today — задачи на сегодня\n"
        "📆 /week — задачи на неделю\n"
        "📋 /list — все задачи\n"
        "✅ /done N — выполнить задачу №N\n"
        "🗑️ /delete N — удалить задачу №N\n"
        "🔍 /search — найти похожие задачи\n\n"

        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "<b>💡 ПОЛЕЗНЫЕ СВЕДЕНИЯ:</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "• <b>Формат дат:</b> <code>DD.MM.YYYY HH:MM</code>\n"
        "• <b>Нумерация:</b> По порядку времени выполнения\n"
        "• <b>Автоочистка:</b> Просроченные задачи удаляются автоматически\n\n"

        "<i>Используйте /help для подробной справки со всеми командами! 📚</i>"
    )
    await message.answer(msg)


async def on_help(message: Message):
    """Показать подробную справку по всем командам"""
    help_text = (
        "🤖 <b>Telegram Planner Bot</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"

        "📝 <b>УПРАВЛЕНИЕ ЗАДАЧАМИ</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "➕ <b>/add</b> - Добавить новую задачу\n"
        "   Формат: <code>DD.MM.YYYY HH:MM</code>\n"
        "   Пример: <code>25.12.2024 14:30</code>\n\n"

        "📅 <b>/today</b> - Задачи на сегодня\n"
        "📆 <b>/week</b> - Задачи на неделю\n"
        "📋 <b>/list</b> - Все задачи пользователя\n\n"

        "✅ <b>/done N</b> - Выполнить задачу №N\n"
        "↩️ <b>/undo N</b> - Отменить выполнение задачи №N\n"
        "🗑️ <b>/delete N</b> - Удалить задачу №N\n\n"

        "🔍 <b>/search запрос</b> - Найти похожие задачи\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"

        "🧹 <b>ОЧИСТКА И СЕРВИС</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🧽 <b>/cleanup</b> - Удалить просроченные задачи\n"
        "💥 <b>/clear_all</b> - Удалить ВСЕ задачи (с подтверждением)\n"
        "🔄 <b>/reset_ids</b> - Сбросить счетчик ID задач\n"
        "❌ <b>/cancel</b> - Отменить текущую операцию\n\n"

        "❓ <b>/help</b> - Показать эту справку\n"
        "🚀 <b>/start</b> - Краткая справка и приветствие\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"

        "💡 <b>ВАЖНЫЕ ЗАМЕЧАНИЯ:</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "• <b>Нумерация:</b> Глобальная по времени выполнения\n"
        "• <b>Формат дат:</b> Строго <code>DD.MM.YYYY HH:MM</code>\n"
        "• <b>Автоочистка:</b> Просроченные задачи удаляются автоматически\n"
        "• <b>FSM:</b> Диалоги поддерживают состояния и отмену\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"

        "🎯 <b>ПРИМЕРЫ ИСПОЛЬЗОВАНИЯ:</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "<b>Добавление задачи:</b>\n"
        "<code>/add</code>\n"
        "└─ <i>Название?</i> → <code>Встреча с клиентом</code>\n"
        "└─ <i>Дата и время?</i> → <code>25.12.2024 14:30</code>\n\n"

        "<b>Управление задачами:</b>\n"
        "<code>/done 1</code> - Выполнить первую задачу\n"
        "<code>/delete 2</code> - Удалить вторую задачу\n\n"

        "<b>Просмотр задач:</b>\n"
        "<code>/today</code> - Задачи на сегодня\n"
        "<code>/week</code> - Задачи на неделю\n"
        "<code>/list</code> - Все задачи\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"

        "📞 <b>ПОДДЕРЖКА:</b> Если возникли проблемы, проверьте:\n"
        "• Правильность формата даты и времени\n"
        "• Наличие активных задач\n"
        "• Корректность номеров задач\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )
    await message.answer(help_text, parse_mode="HTML")


async def on_add(message: Message, state: FSMContext):
    """Начало диалога добавления задачи"""
    await state.clear()  # Очищаем предыдущее состояние

    await message.answer(
        "📝 <b>Добавление новой задачи</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "✏️ <b>Шаг 1:</b> Введите <b>название задачи</b>\n\n"
        "💡 <i>Например: Встреча с клиентом, Сделать домашнее задание</i>\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )
    await state.set_state(AddTaskStates.waiting_for_title)


async def process_title(message: Message, state: FSMContext):
    """Обработка названия задачи"""
    title = message.text.strip()

    if len(title) < 1:
        await message.answer(
            "❌ <b>Пустое название!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "📝 Название задачи не может быть пустым!\n\n"
            "✏️ <b>Введите название задачи:</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return

    if len(title) > 200:
        await message.answer(
            "❌ <b>Слишком длинное название!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"📏 Длина: {len(title)} символов (макс. 200)\n\n"
            "✏️ <b>Введите более короткое название:</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return

    # Сохраняем название в состоянии
    await state.update_data(title=title)

    await message.answer(
        f"✅ <b>Название принято:</b> <i>{title}</i>\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "📅 <b>Шаг 2:</b> Введите <b>дату и время</b>\n\n"
        "📝 <b>Формат:</b> <code>DD.MM.YYYY HH:MM</code>\n\n"
        "💡 <b>Примеры:</b>\n"
        "• <code>25.12.2024 14:30</code> - Рождество\n"
        "• <code>01.01.2025 09:00</code> - Новый год\n"
        "• <code>15.03.2024 16:45</code> - Встреча\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )
    await state.set_state(AddTaskStates.waiting_for_datetime)


async def process_datetime(message: Message, state: FSMContext):
    """Обработка даты и времени и создание задачи"""
    datetime_input = message.text.strip()

    # Получаем сохраненное название
    data = await state.get_data()
    title = data.get('title')

    # Разделяем дату и время (ожиддаем формат "DD.MM.YYYY HH:MM")
    parts = datetime_input.split()
    if len(parts) != 2:
        await message.answer(
            "❌ <b>Неверный формат ввода!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"📝 Вы ввели: <code>{datetime_input}</code>\n\n"
            "📋 <b>Ожидаемый формат:</b> <code>DD.MM.YYYY HH:MM</code>\n\n"
            "💡 <b>Примеры:</b>\n"
            "• <code>25.12.2024 14:30</code> ✅\n"
            "• <code>01.01.2025 09:00</code> ✅\n\n"
            "🔄 <b>Повторите ввод в правильном формате:</b>"
        )
        await state.set_state(AddTaskStates.waiting_for_datetime)
        return

    date_input, time_input = parts

    # Парсим дату и время
    parsed = utils.parse_date_time(date_input, time_input)
    if parsed is None:
        await message.answer(
            "❌ <b>Некорректная дата или время!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"📝 Вы ввели: <code>{date_input} {time_input}</code>\n\n"
            "📅 <b>Проверьте формат:</b> <code>DD.MM.YYYY HH:MM</code>\n\n"
            "💡 <b>Примеры корректных дат:</b>\n"
            "• <code>25.12.2024 14:30</code> ✅\n"
            "• <code>01.01.2025 09:15</code> ✅\n\n"
            "🔄 <b>Введите правильную дату и время:</b>"
        )
        await state.set_state(AddTaskStates.waiting_for_datetime)
        return

    date_str, time_str = parsed

    # Проверяем, что время не в прошлом
    if not utils.validate_datetime(date_str, time_str):
        await message.answer(
            "⏰ <b>Ошибка: Прошедшее время!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "❌ Нельзя добавить задачу на прошедшее время!\n\n"
            f"📅 Вы ввели: <code>{date_str} {time_str}</code>\n"
            f"📅 Текущее время: <code>{utils.current_date()} {utils.current_time()}</code>\n\n"
            "💡 <b>Выберите дату и время в будущем</b>\n\n"
            "📝 <b>Формат:</b> <code>DD.MM.YYYY HH:MM</code>\n\n"
            "🔄 <b>Повторите ввод:</b>"
        )
        await state.set_state(AddTaskStates.waiting_for_datetime)
        return  # НЕ очищаем состояние, даем возможность ввести заново

    # Создаем задачу
    vec = utils.make_embedding(title)
    blob = utils.emb_to_blob(vec)
    task_id = await database.insert_task(message.from_user.id, title, date_str, time_str, blob)

    formatted_datetime = utils.format_datetime_display(date_str, time_str)
    await message.answer(
        "🎉 <b>Задача успешно создана!</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🆔 <b>ID:</b> {task_id}\n"
        f"📝 <b>Задача:</b> {title}\n"
        f"📅 <b>Когда:</b> <code>{formatted_datetime}</code>\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"💡 <i>Используйте /done {task_id} когда выполните задачу</i>"
    )

    # Очищаем состояние
    await state.clear()


async def cancel_add(message: Message, state: FSMContext):
    """Отмена добавления задачи"""
    await state.clear()
    await message.answer(
        "❌ <b>Операция отменена</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "🚫 <i>Текущая операция прервана</i>\n\n"
        "💡 <i>Используйте /help для просмотра команд</i>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )


async def on_today(message: Message):
    # Сначала удаляем просроченные задачи
    deleted_count = await database.delete_expired_tasks()
    if deleted_count > 0:
        log.info(f"Удалено {deleted_count} просроченных задач")

    today = utils.current_date()
    tasks = await database.fetch_tasks_for_date(message.from_user.id, today)

    if not tasks:
        await message.answer(
            "<b>Задачи на сегодня</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "<i>На сегодня задач нет! Отличный день для отдыха</i>\n\n"
            "<i>Используйте /add чтобы добавить задачу</i>"
        )
    else:
        formatted_date = utils.format_date_display(today)
        lines = [
            f"<b>Задачи на {formatted_date}</b>",
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        ]

        for i, (task_id, title, time_str, status) in enumerate(tasks, 1):
            if status == "done":
                mark = "[ВЫПОЛНЕНО]"
                emoji = "☑️"
            else:
                mark = "[НЕ ВЫПОЛНЕНО]"
                emoji = "📝"

            formatted_datetime = utils.format_datetime_display(today, time_str)
            lines.append(f"{emoji} <b>{i}.</b> <code>{formatted_datetime}</code> - {title} {mark}")

        lines.extend([
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
            f"<i>Используйте /done N для выполнения задачи</i>"
        ])

        await message.answer("\n".join(lines))


async def on_week(message: Message):
    """Показать задачи на неделю вперед"""
    # Сначала удаляем просроченные задачи
    deleted_count = await database.delete_expired_tasks()
    if deleted_count > 0:
        log.info(f"Удалено {deleted_count} просроченных задач")

    today = datetime.now().date()
    week_dates = [(today + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(7)]

    tasks = await database.fetch_tasks_for_dates(message.from_user.id, week_dates)

    if not tasks:
        await message.answer(
            "📆 <b>Задачи на неделю</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "✅ <i>На этой неделе задач нет! Свободное время 🎉</i>\n\n"
            "💡 <i>Используйте /add чтобы запланировать дела</i>"
        )
        return

    # Группируем по датам
    tasks_by_date = {}
    for task_id, title, date_str, time_str, status in tasks:
        if date_str not in tasks_by_date:
            tasks_by_date[date_str] = []
        tasks_by_date[date_str].append((task_id, title, time_str, status))

    lines = [
        "📆 <b>Задачи на неделю</b>",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ]

    for date_str in sorted(tasks_by_date.keys()):
        formatted_date = utils.format_date_display(date_str)
        lines.append(f"\n📅 <b>{formatted_date}:</b>")

        for i, (task_id, title, time_str, status) in enumerate(tasks_by_date[date_str], 1):
            if status == "done":
                mark = "✅"
                emoji = "☑️"
            else:
                mark = "⏳"
                emoji = "📝"

            formatted_datetime = utils.format_datetime_display(date_str, time_str)
            lines.append(f"  {emoji} <b>{i}.</b> <code>{formatted_datetime}</code> - {title} {mark}")

    lines.extend([
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        "💡 <i>Номера задач локальные в пределах дня</i>"
    ])

    await message.answer("\n".join(lines))


async def on_list(message: Message):
    """Показать все задачи пользователя"""
    # Сначала удаляем просроченные задачи
    deleted_count = await database.delete_expired_tasks()
    if deleted_count > 0:
        await message.answer(f"🗑️ Удалено {deleted_count} просроченных задач")

    tasks = await database.fetch_all_tasks(message.from_user.id)

    if not tasks:
        await message.answer(
            "📝 <b>Все ваши задачи</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "📭 <i>У вас пока нет задач</i>\n\n"
            "💡 <i>Используйте /add чтобы добавить первую задачу</i>"
        )
        return

    lines = [
        "📋 <b>Все ваши задачи</b>",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ]

    # Группируем по статусу с порядковой нумерацией
    pending_tasks = []
    done_tasks = []

    task_number = 1
    for task_id, title, date_str, time_str, status in tasks:
        formatted_datetime = utils.format_datetime_display(date_str, time_str)

        if status == "done":
            emoji = "☑️"
            mark = "✅"
            done_tasks.append(f"  {emoji} <b>{task_number}.</b> <code>{formatted_datetime}</code> - {title} {mark}")
        else:
            emoji = "📝"
            mark = "⏳"
            pending_tasks.append(f"  {emoji} <b>{task_number}.</b> <code>{formatted_datetime}</code> - {title} {mark}")

        task_number += 1

    # Показываем сначала невыполненные задачи
    if pending_tasks:
        lines.extend([
            f"\n🔄 <b>Невыполненные задачи ({len(pending_tasks)}):</b>",
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        ])
        lines.extend(pending_tasks[:20])  # Ограничиваем количество

    if done_tasks:
        lines.extend([
            f"\n✅ <b>Выполненные задачи ({len(done_tasks)}):</b>",
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        ])
        lines.extend(done_tasks[:10])  # Меньше выполненных показываем

    lines.extend([
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"💡 <i>Используйте /done N для выполнения задач</i>"
    ])

    if len(pending_tasks) > 20 or len(done_tasks) > 10:
        lines.append(f"💡 <i>Показаны не все задачи. Используйте /today или /week для фильтрации</i>")

    await message.answer("\n".join(lines))


async def on_cleanup(message: Message):
    """Удалить все просроченные задачи"""
    deleted_count = await database.delete_expired_tasks()

    if deleted_count > 0:
        await message.answer(
            f"🧽 <b>Очистка завершена!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"🗑️ Удалено просроченных задач: <b>{deleted_count}</b>\n\n"
            "✅ <i>Список задач очищен от старых задач</i>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
    else:
        await message.answer(
            "🧽 <b>Очистка списка задач</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "✅ <i>Просроченных задач не найдено</i>\n\n"
            "🎉 <i>Ваш список задач в порядке!</i>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )


async def on_reset_ids(message: Message):
    """Сбросить счетчик ID задач"""
    task_count = await database.count_user_tasks(message.from_user.id)

    if task_count == 0:
        # Если задач нет, сбрасываем счетчик на 1
        await database.reset_task_ids()
        await message.answer(
            "🔄 <b>Сброс ID счетчика</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "✅ ID счетчик сброшен на 1\n"
            "📭 Активных задач: 0\n\n"
            "💡 <i>Новые задачи будут иметь ID начиная с 1</i>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return

    # Если задачи есть, устанавливаем счетчик на следующий после максимального ID
    await database.reset_task_ids()
    await message.answer(
        f"🔄 <b>Оптимизация ID счетчика</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"✅ ID счетчик оптимизирован\n"
        f"📊 Активных задач: <b>{task_count}</b>\n\n"
        f"💡 <i>Новые задачи будут иметь последовательные ID</i>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )


async def on_clear_all(message: Message, state: FSMContext):
    """Начать процесс удаления всех задач с подтверждением"""
    # Проверяем, есть ли задачи у пользователя
    task_count = await database.count_user_tasks(message.from_user.id)

    if task_count == 0:
        await message.answer(
            "📭 <b>Удаление всех задач</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "❌ <i>У вас нет задач для удаления</i>\n\n"
            "💡 <i>Добавьте задачи с помощью /add</i>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return

    # Предупреждаем пользователя и запрашиваем подтверждение
    await message.answer(
        f"⚠️ <b>КРИТИЧЕСКОЕ ПРЕДУПРЕЖДЕНИЕ!</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"💥 Вы собираетесь удалить <b>ВСЕ {task_count} задач</b>!\n\n"
        f"🔴 <b>ЭТО ДЕЙСТВИЕ НЕЛЬЗЯ ОТМЕНИТЬ!</b>\n\n"
        f"📊 Будет удалено:\n"
        f"• Все выполненные задачи\n"
        f"• Все невыполненные задачи\n"
        f"• Вся история планирования\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"✅ <b>Подтверждение:</b>\n"
        f"Введите: <code>ДА УДАЛИТЬ ВСЕ</code>\n\n"
        f"❌ <b>Отмена:</b>\n"
        f"Введите: <code>НЕТ</code> или <code>/cancel</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    )

    # Сохраняем информацию о количестве задач для подтверждения
    await state.update_data(task_count=task_count)
    await state.set_state(ClearAllStates.waiting_for_confirmation)


async def process_clear_confirmation(message: Message, state: FSMContext):
    """Обработка подтверждения удаления всех задач"""
    confirmation = message.text.strip()

    # Явная проверка на отмену
    if confirmation.lower() in ["/cancel", "cancel", "отмена", "отменить"]:
        await cancel_add(message, state)
        return

    confirmation = confirmation.upper()

    if confirmation == "ДА УДАЛИТЬ ВСЕ":
        # Получаем сохраненное количество задач
        data = await state.get_data()
        expected_count = data.get('task_count', 0)

        # Удаляем все задачи
        deleted_count = await database.delete_all_tasks(message.from_user.id)

        await message.answer(
            f"💥 <b>ВСЕ ЗАДАЧИ УДАЛЕНЫ!</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"🗑️ Удалено задач: <b>{deleted_count}</b>\n\n"
            f"🔄 ID счетчик сброшен на 1\n\n"
            f"✅ <i>База данных полностью очищена</i>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"💡 <i>Используйте /add чтобы добавить новые задачи</i>"
        )

        # Очищаем состояние
        await state.clear()

    elif confirmation in ["НЕТ", "NO", "CANCEL", "ОТМЕНА", "НЕТ НЕ УДАЛЯТЬ", "ОТМЕНИТЬ"]:
        await message.answer(
            "✅ <b>Удаление отменено</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "🎉 <i>Все задачи сохранены</i>\n\n"
            "💡 <i>Используйте /list чтобы посмотреть задачи</i>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        await state.clear()

    else:
        await message.answer(
            "❌ <b>Неверное подтверждение!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "📝 <b>Доступные варианты:</b>\n\n"
            "✅ <b>Подтвердить:</b> <code>ДА УДАЛИТЬ ВСЕ</code>\n"
            "❌ <b>Отменить:</b> <code>НЕТ</code>, <code>ОТМЕНА</code> или <code>/cancel</code>\n\n"
            "🔄 Повторите ввод:\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        await state.set_state(ClearAllStates.waiting_for_confirmation)


async def on_done(message: Message):
    # формат: /done ID (где ID - порядковый номер в списке задач)
    try:
        display_id = int(message.text.split(" ", 1)[1])
    except (ValueError, IndexError):
        await message.answer("Нужно указать номер задачи.\nПример: /done 1")
        return

    # Получаем все задачи пользователя, отсортированные по времени
    tasks = await database.fetch_all_tasks(message.from_user.id, 1000)  # Больше лимит для поиска

    if display_id < 1 or display_id > len(tasks):
        await message.answer(f"Задача с номером {display_id} не найдена.")
        return

    # Находим задачу по порядковому номеру
    real_task_id = tasks[display_id - 1][0]  # Первый элемент - это ID в базе

    count = await database.mark_task_done(message.from_user.id, real_task_id)
    if count > 0:
        await message.answer(
            f"✅ <b>Задача выполнена!</b>\n\n"
            f"🎯 Задача №{display_id} отмечена как выполненная\n\n"
            f"💡 <i>Используйте /undo {display_id} если передумали</i>"
        )
    else:
        await message.answer(
            "❌ <b>Ошибка выполнения задачи</b>\n\n"
            "Возможные причины:\n"
            "• Задача уже выполнена\n"
            "• Задача не найдена\n"
            "• Неверный номер задачи\n\n"
            "💡 <i>Проверьте список задач с помощью /list</i>"
        )


async def on_undo(message: Message):
    # формат: /undo ID (где ID - порядковый номер в списке задач)
    try:
        display_id = int(message.text.split(" ", 1)[1])
    except (ValueError, IndexError):
        await message.answer("Нужно указать номер задачи.\nПример: /undo 1")
        return

    # Получаем все задачи пользователя, отсортированные по времени
    tasks = await database.fetch_all_tasks(message.from_user.id, 1000)

    if display_id < 1 or display_id > len(tasks):
        await message.answer(f"Задача с номером {display_id} не найдена.")
        return

    # Находим задачу по порядковому номеру
    real_task_id = tasks[display_id - 1][0]

    count = await database.mark_task_undo(message.from_user.id, real_task_id)
    if count > 0:
        await message.answer(
            f"↩️ <b>Задача возвращена!</b>\n\n"
            f"🔄 Задача №{display_id} снова активна\n\n"
            f"💡 <i>Используйте /done {display_id} чтобы выполнить её снова</i>"
        )
    else:
        await message.answer(
            "❌ <b>Ошибка отмены выполнения</b>\n\n"
            "Возможные причины:\n"
            "• Задача ещё не выполнена\n"
            "• Задача не найдена\n"
            "• Неверный номер задачи\n\n"
            "💡 <i>Проверьте список задач с помощью /list</i>"
        )


async def on_delete(message: Message):
    """Удалить задачу"""
    try:
        display_id = int(message.text.split(" ", 1)[1])
    except (ValueError, IndexError):
        await message.answer("Нужно указать номер задачи.\nПример: /delete 1")
        return

    # Получаем все задачи пользователя, отсортированные по времени
    tasks = await database.fetch_all_tasks(message.from_user.id, 1000)

    if display_id < 1 or display_id > len(tasks):
        await message.answer(f"Задача с номером {display_id} не найдена.")
        return

    # Находим задачу по порядковому номеру
    real_task_id = tasks[display_id - 1][0]

    count = await database.delete_task(message.from_user.id, real_task_id)
    if count > 0:
        await message.answer(
            f"🗑️ <b>Задача удалена!</b>\n\n"
            f"❌ Задача №{display_id} полностью удалена\n\n"
            f"⚠️ <i>Это действие нельзя отменить</i>"
        )
    else:
        await message.answer(
            "❌ <b>Ошибка удаления задачи</b>\n\n"
            "Возможные причины:\n"
            "• Задача не найдена\n"
            "• Неверный номер задачи\n"
            "• Задача уже удалена\n\n"
            "💡 <i>Проверьте список задач с помощью /list</i>"
        )


async def on_search(message: Message):
    """Поиск похожих задач по тексту"""
    if " " not in message.text:
        await message.answer(
            "🔍 <b>Поиск задач</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "❌ <i>Укажите текст для поиска</i>\n\n"
            "💡 <b>Пример:</b> <code>/search купить</code>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return

    query_text = message.text.split(" ", 1)[1]

    # Проверяем длину запроса
    if len(query_text.strip()) < 2:
        await message.answer(
            "🔍 <b>Поиск задач</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "❌ <i>Запрос слишком короткий</i>\n\n"
            "💡 <i>Введите минимум 2 символа для поиска</i>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return

    query_vec = utils.make_embedding(query_text)

    # Получаем все задачи пользователя (включая старые)
    rows = await database.load_tasks_with_vectors(message.from_user.id)
    if not rows:
        await message.answer(
            "🔍 <b>Поиск задач</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "📭 <i>У вас нет задач для поиска</i>\n\n"
            "💡 <i>Добавьте задачи с помощью /add</i>\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return

    # Считаем косинусное сходство
    results = []
    for t_id, t_title, t_blob in rows:
        t_vec = utils.blob_to_emb(t_blob)
        if t_vec is not None:
            score = utils.cosine_sim(query_vec, t_vec)
            results.append((score, t_id, t_title))

    # Сортируем по убыванию похожести
    results.sort(key=lambda x: x[0], reverse=True)

    if not results or results[0][0] < 0.1:  # минимальный порог похожести
        await message.answer(
            f"🔍 <b>Поиск: '{query_text}'</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"❌ <i>Ничего похожего не найдено</i>\n\n"
            f"💡 <i>Попробуйте другие ключевые слова</i>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return

    # Берем топ результатов, но показываем только релевантные
    top_results = [r for r in results if r[0] >= 0.3][:5]  # максимум 5 результатов

    if not top_results:
        # Если нет очень похожих, показываем хотя бы одну самую близкую
        top_results = results[:1]

    lines = [
        f"🔍 <b>Поиск: '{query_text}'</b>",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ]

    for score, task_id, title in top_results:
        if score >= 0.8:
            similarity_icon = "🎯"
            similarity_text = "Отлично"
        elif score >= 0.6:
            similarity_icon = "✅"
            similarity_text = "Хорошо"
        elif score >= 0.4:
            similarity_icon = "⚠️"
            similarity_text = "Средне"
        else:
            similarity_icon = "🤔"
            similarity_text = "Слабо"

        lines.append(f"{similarity_icon} <b>№{task_id}:</b> {title}")
        if len(top_results) > 1:
            lines.append(f"   📊 Сходство: {similarity_text} ({score:.1%})")

    # Если нашли очень похожую задачу, предлагаем действия
    if results[0][0] >= 0.8:
        best_match_id = results[0][1]
        lines.extend([
            "",
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
            f"💡 <b>Рекомендация:</b>",
            f"🎯 Задача №{best_match_id} очень похожа на ваш запрос",
            f"✅ Отправьте: <code>/done {best_match_id}</code> чтобы выполнить"
        ])

    lines.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")

    await message.answer("\n".join(lines))
